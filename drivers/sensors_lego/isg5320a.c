...
#endif
#if defined(CONFIG_CERTIFY_HALL)
...
#if defined(CONFIG_SENSORS_HALL) || defined(CONFIG_CERTIFY_HALL)
...
static int isg5320a_i2c_write(struct isg5320a_data *data, u8 cmd, u8 *val)
{
	...
	while (retry < 3) {
		...
		if (ret == 1)
			break;
		pr_err("%s %s: i2c_transfer failed(%d) retry : %d\n", ISG5320A_TAG, __func__, ret, retry);
...
static int isg5320a_setup_reg(struct isg5320a_data *data)
{
	...
	pos = 0;
	ret = fp->f_op->read(fp, (char __user *)file_data, file_size, &pos);
	if (ret != file_size) {
	...
	for (i = 0; i < file_size; i++) {
		if (file_data[i] == 'W') {
			...
			ret = isg5320a_i2c_write_one(data, addr, val);
			if (ret < 0) {
				pr_info("%s %s - i2c write fail(%d)", ISG5320A_TAG, __func__, ret);
				return ret;
			}
			pr_info("%s W %02X %02X\n", ISG5320A_TAG, addr, val);
			ret = isg5320a_i2c_read(data, addr, &val, 1);
			if (ret < 0) {
				pr_info("%s %s - i2c read fail(%d)", ISG5320A_TAG, __func__, ret);
				return ret;
			}
			pr_info("%s   %02X %02X\n", ISG5320A_TAG, addr, val);
...
static void isg5320a_initialize(struct isg5320a_data *data)
{
	...
	if (ret < 0 && data->setup_reg_exist) {
		for (i = 0; i < data->reg_size ; i++) {
			...
			{
				pr_info("%s %02X %02X\n", ISG5320A_TAG, data->setup_reg[index],
...
/*
			...
			pr_info("%s   %02X %02X\n", ISG5320A_TAG, data->setup_reg[index], val);
*/
#endif
		}
	}
	...
	if (pdata->initialized == ON) {
		...
		case USB_STATUS_NOTIFY_DETACH:
			pr_info("%s - drp = %d attat = %d\n", ISG5320A_TAG, usb_status.drp,
				usb_status.attach);
			isg5320a_force_calibration(pdata, false);
			break;
		default:

		...
		struct sm5714_policy_data *policy)
{
	...
	/* ST_PE_SNK_WAIT_CAP */
	if (sm5714_usbpd_wait_msg(pd_data, MSG_SRC_CAP, tSinkWaitCap))
		...
		struct sm5714_policy_data *policy)
{
	...
	if (policy->last_state != policy->state) {
		...
		msg = sm5714_usbpd_wait_msg(pd_data, MSG_ACCEPT | MSG_REJECT
		...
		struct sm5714_policy_data *policy)
{
	...
	if (policy->last_state != policy->state) {
		...
		if (sm5714_usbpd_wait_msg(pd_data, MSG_PSRDY, tPSSourceOn)) {
		...
		struct sm5714_policy_data *policy)
{
	...
	pd_data->phy_ops.get_power_role(pd_data, &power_role);
	/* ST_PE_VC_SWAP_WAIT */
	if (sm5714_usbpd_wait_msg(pd_data, MSG_PSRDY, tVCONNSourceTimeout)) {
		pd_data->counter.swap_hard_reset_counter = 0;
		dev_info(pd_data->dev, "got PSRDY.\n");
		...
		struct sm5714_policy_data *policy)
{
	...
	if (policy->last_state != policy->state) {
		...
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
			if (sm5714_usbpd_wait_msg(pd_data, VDM_DISCOVER_SVID, tVDMSenderResponse)) {
				if (policy->rx_data_obj[0].structured_vdm.command_type == Responder_ACK)
		...
		struct sm5714_policy_data *policy)
{
	...
	if (policy->last_state != policy->state) {
		...
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
			if (sm5714_usbpd_wait_msg(pd_data, VDM_DISCOVER_MODE, tVDMSenderResponse)) {
				if (policy->rx_data_obj[0].structured_vdm.command_type == Responder_ACK) {
		...
		struct sm5714_policy_data *policy)
{
	...
	if (policy->last_state != policy->state) {
		...
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
			if (sm5714_usbpd_wait_msg(pd_data, VDM_ENTER_MODE, tVDMWaitModeEntry)) {
				if (policy->rx_data_obj[0].structured_vdm.command_type == Responder_ACK)
		...
		struct sm5714_policy_data *policy)
{
	...
	if (policy->last_state != policy->state) {
		...
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
			if (sm5714_usbpd_wait_msg(pd_data, VDM_EXIT_MODE, tVDMWaitModeExit)) {
				if (policy->rx_data_obj[0].structured_vdm.command_type == Responder_ACK)
		...
		struct sm5714_policy_data *policy)
{
	...
	if (policy->last_state != policy->state) {
		...
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
			if (sm5714_usbpd_wait_msg(pd_data, VDM_DP_STATUS_UPDATE, tVDMSenderResponse)) {
	...
	struct sm5714_policy_data *policy)
{
	...
	if (policy->last_state != policy->state) {
		...
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
			if (sm5714_usbpd_wait_msg(pd_data, VDM_DP_CONFIGURE, tVDMSenderResponse)) {
				if (policy->rx_data_obj[0].structured_vdm.command_type == Responder_ACK) {
					if (manager->SVID_DP == PD_SID_1) {

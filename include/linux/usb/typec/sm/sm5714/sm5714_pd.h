...
typedef enum sm5714_usbpd_manager_command {
...
} sm5714_usbpd_manager_command_type;

typedef enum sm5714_usbpd_manager_event {
...
enum sm5714_usbpd_protocol_status {
	DEFAULT_PROTOCOL_NONE	= 0,
	MESSAGE_SENT			= 1,
	TRANSMISSION_ERROR		= 2
};

enum sm5714_usbpd_policy_informed {
...
struct sm5714_usbpd_manager_data {
	sm5714_usbpd_manager_command_type cmd;  /* request to policy engine */
	sm5714_usbpd_manager_event_type   event;    /* policy engine infromed */
...
static inline struct sm5714_usbpd_data *protocol_rx_to_usbpd(
		struct sm5714_protocol_data *rx)
{
	return container_of(rx, struct sm5714_usbpd_data, protocol_rx);
}

static inline struct sm5714_usbpd_data *protocol_tx_to_usbpd(
		struct sm5714_protocol_data *tx)
{
	return container_of(tx, struct sm5714_usbpd_data, protocol_tx);
}

static inline struct sm5714_usbpd_data *policy_to_usbpd(
		struct sm5714_policy_data *policy)
{
	return container_of(policy, struct sm5714_usbpd_data, policy);
}

static inline struct sm5714_usbpd_data *manager_to_usbpd(
		struct sm5714_usbpd_manager_data *manager)
{
...
extern bool sm5714_usbpd_power_role_swap(struct sm5714_usbpd_data *pd_data);
extern bool sm5714_usbpd_vconn_source_swap(struct sm5714_usbpd_data *pd_data);
extern void sm5714_usbpd_turn_on_source(struct sm5714_usbpd_data *pd_data);
extern void sm5714_usbpd_turn_off_power_supply(
		struct sm5714_usbpd_data *pd_data);
extern void sm5714_usbpd_turn_off_power_sink(
		struct sm5714_usbpd_data *pd_data);
extern bool sm5714_usbpd_data_role_swap(struct sm5714_usbpd_data *pd_data);
extern int sm5714_usbpd_get_identity(struct sm5714_usbpd_data *pd_data);
extern int sm5714_usbpd_get_svids(struct sm5714_usbpd_data *pd_data);
extern int sm5714_usbpd_get_modes(struct sm5714_usbpd_data *pd_data);
